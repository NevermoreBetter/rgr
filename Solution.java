...
public class Solution {
    ...
    static double[][] pivotize(double[][] m) { //P - масив індексів, у вигляді матриці
        int n = m.length;
    ...
    public static double[] calculate(double[][] array) {// Окремий метод для підрахунку рішень
        ...
        for (int k = 0; k < array[0].length - 1; k++) {
            ...
            if (max == 0) {
                throw new IllegalArgumentException("Матриця не повинна бути виродженою"); // кв. матриця, визначник якої = 0
            }

            //змінюємо місцями номера рядків, які будуть представлені
            int value = p[k];
            p[k] = p[numberRowForReplace];
            p[numberRowForReplace] = value;

            //переставляємо відповідні рядки
            double[] row = array[k];
            ...
            for (int i = k + 1; i < array.length; i++) {
                //ділимо елементи нище гловної діагоналі на ведущий?
                array[i][k] = array[i][k] / array[k][k];
                for (int j = k + 1; j < array.length; j++) {
                    //вичитуємо елементи поточного k-рядка помножені на коефіцієнт із елементів рядків нище нього
